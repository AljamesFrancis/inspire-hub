...
export default function TimeSlotSchedule() {
  ...
  const newSlots = slots.map((slot) => {
    ...
    // Mark selected only if within range and available
    if (
      slotHour >= startHour &&
      slotHour < endHour &&
    ...
    // Reset if was selected but now not in range
    if (
  ...
  // Validation for overlapping with busy or after-hours
  const invalidOverlap = slots.some((slot) => {
    const slotHour = parseInt(slot.time);
    return (
      slotHour >= startHour &&
      slotHour < endHour &&
  ...
  const handleSubmit = (e) => {
    ...
    alert(
      `Reservation details:\nName: ${formData.name}\nEmail: ${formData.email}\nDate: ${formData.date}\nTime (military): ${militaryTime}\nDuration: ${formData.duration}\nGuests: ${formData.guests}\nSpecial Requests: ${formData.specialRequests}\nSelected Slots: ${selectedSlots || "None selected"}`
    );
  };
